...
declare variable $search-string-file := doc('../pre-voyant-search-strings-languages.tsv');
...
declare variable $require-existing-vowels := 1;
...
declare function scholarNET:addPrefixes($search-string as xs:string) as xs:string {
    if ($add-optional-prefixes) then 
            let $prefixes-to-add := concat(string-join($prefixes,'?'),'?')
...
(:            return (scholarNET:voyantSearchString($cell)):)
               return (scholarNET:addPrefixes(scholarNET:addArticle($cell)))
